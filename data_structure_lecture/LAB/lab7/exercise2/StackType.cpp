...
void StackType::Copy(const StackType& anotherStack)
{
    ...
    //copy last values
    NodeType* push_interator = this->topPtr;
    iterator = iterator->next; // 두번째 value 부터
    while (iterator != NULL)
    {
        push_interator->next = new NodeType;
        push_interator = push_interator->next;
        push_interator->info = iterator->info;
        iterator = iterator->next;
    }
    push_interator->next = NULL;
    return;
}